...
# ─── 상수 ───
# ─── 블로그 홍보 문구 ───
BLOG_CTA_NARRATION = "당신의 경험을 댓글로 공유 해주세요!"
...
배경 영상 키워드 규칙:
search_keywords는 Pexels에서 검색할 영어 키워드 4개를 배열로 생성하세요:
- 각 키워드는 1~3단어의 영어 (예: "brain neuron", "thinking person", "romantic heterosexual couple", "morning sunlight")
- 영상 내용의 4단계 흐름(후킹, 공감, 원리, 해결)과 맞는 키워드
...
- 4개가 서로 다른 느낌이어야 함 (다양한 배경 전환을 위해)
  ...
  "tiktok_hashtags": ["#주제태그1", "#주제태그2", "#주제태그3", "#영어태그1"]
    ...
    def generate(self, category_id=None, weekday=None, language='ko', save_history=True):
        ...
        comment_instruction = (
            ...
            "   - 댓글형은 \"선택 질문 + 이유 한 줄 요청\" 2단 구조를 한 호흡으로 담으세요(2문장 연타 금지).\n"
